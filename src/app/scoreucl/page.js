...
import overlayUcl from "@/data/overlayUcl";
    ...
    <main className="flex flex-col items-center justify-center min-h-screen p-6 bg-gray-100">
      ...
      <div className="flex gap-4 mb-6">
        <div>
          <p className="mb-2 font-semibold">Tim 1</p>
          <select
            onChange={(e) => setOverlayImage(`/assets/ucl/${e.target.value}`)}
            className="border px-3 py-2 rounded"
          >
            <option value="">-- Pilih Gambar --</option>
            {Object.entries(overlayUcl).map(([liga, teams]) => (
              <optgroup key={liga} label={liga}>
                {teams.map((team) => (
                  <option key={team} value={`${liga}/${team}`}>
                    {team.replace(".png", "")}
        ...
        <div>
          <p className="mb-2 font-semibold">Tim 2</p>
            <select
              onChange={(e) => setOverlayImage2(`/assets/ucl/${e.target.value}`)}
              className="border px-3 py-2 rounded"
            >
              <option value="">-- Pilih Gambar --</option>
              {Object.entries(overlayUcl).map(([liga, teams]) => (
