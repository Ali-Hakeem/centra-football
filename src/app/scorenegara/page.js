...
import overlayWorldCup from "@/data/overlayWorldCup";
    ...
    <main className="flex flex-col items-center justify-center min-h-screen p-6 bg-gray-100">
      ...
      <div className="flex gap-4 mb-6">
        <div>
          ...
          <select
            onChange={(e) => setOverlayImage(`/assets/nation/${e.target.value}`)}
            className="border px-3 py-2 rounded"
          >
            <option value="">-- Pilih Gambar --</option>
            {Object.entries(overlayWorldCup).map(([liga, teams]) => (
              <optgroup key={liga} label={liga}>
                {teams.map((team) => (
                  <option key={team} value={`${liga}/${team}`}>
                    {team.replace(".png", "")}
        ...
        <div>
          <p className="mb-2 font-semibold">Tim 2</p>
            <select
              onChange={(e) => setOverlayImage2(`/assets/nation/${e.target.value}`)}
              className="border px-3 py-2 rounded"
            >
              <option value="">-- Pilih Gambar --</option>
              {Object.entries(overlayWorldCup).map(([liga, teams]) => (
